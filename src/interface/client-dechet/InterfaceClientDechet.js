...
import { LinkClientDechets } from "./components/SideBar/LinkClientDechets";
...
export default function InterfaceClientDechet() {
            ...
            <Box bgcolor="background.paper" sx={{ p: 1 }}>
                  <Box sx={{ display: 'flex' }}>
                    ...
                    <Drawer variant="permanent" PaperProps={{sx:{backgroundColor:'primary.main'}}} open={open}>
                            ...
                            <List> {LinkClientDechets.map((lien, i)=> <><MenuItem key={i} item={lien} open={open}/></>)}</List>
                    </Drawer>              
