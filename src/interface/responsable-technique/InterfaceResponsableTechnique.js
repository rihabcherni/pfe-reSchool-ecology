...
import { LinkResponsableTechnique } from "./components/SideBar/LinkResponsableTechnique";
...
export default function InterfaceResponsableTechnique() {
            ...
            <Box bgcolor="background.paper" sx={{ p: 1 }}>
                  <Box sx={{ display: 'flex' }}>
                    ...
                    <Drawer variant="permanent" PaperProps={{sx:{backgroundColor:'primary.main'}}} open={open}>
                            ...
                            <List> {LinkResponsableTechnique.map((lien, key)=> <><MenuItem key={key} item={lien} open={open}/></>)}</List>
                    </Drawer>              
