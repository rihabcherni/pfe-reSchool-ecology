...
import { LinkResponsableCommerciale } from "./components/SideBar/LinkResponsableCommerciale";
...
export default function InterfaceResponsableCommerciale() {
            ...
            <Box bgcolor="background.paper" sx={{ p: 1 }}>
                  <Box sx={{ display: 'flex' }}>
                    ...
                    <Drawer variant="permanent" PaperProps={{sx:{backgroundColor:'primary.main'}}} open={open}>
                            ...
                            <List> {LinkResponsableCommerciale.map((lien, key)=> <><MenuItem key={key} item={lien} open={open}/></>)}</List>
                    </Drawer>              
