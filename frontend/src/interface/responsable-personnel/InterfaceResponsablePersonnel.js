...
import { LinkResponsablePersonnel } from "./components/SideBar/LinkResponsablePersonnel";
...
export default function InterfaceResponsablePersonnel() {
            ...
            <Box bgcolor="background.paper" sx={{ p: 1 }}>
                  <Box sx={{ display: 'flex' }}>
                    ...
                    <Drawer variant="permanent" PaperProps={{sx:{backgroundColor:'primary.main'}}} open={open}>
                            ...
                            <List> {LinkResponsablePersonnel.map((lien, key)=> <><MenuItem key={key} item={lien} open={open}/></>)}</List>
                    </Drawer>              
