...
import { LinkOuvrier } from "./components/SideBar/LinkOuvrier";
...
export default function InterfaceOuvrier() {
            ...
            <Box bgcolor="background.paper" sx={{ p: 1 }}>
                  <Box sx={{ display: 'flex' }}>
                    ...
                    <Drawer variant="permanent" PaperProps={{sx:{backgroundColor:'primary.main'}}} open={open}>
                            ...
                            <List> {LinkOuvrier.map((lien, key)=> <><MenuItem key={key} item={lien} open={open}/></>)}</List>
                    </Drawer>              
