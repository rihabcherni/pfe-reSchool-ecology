...
<html lang="en">
    ...
    <body class="page"  >
        ...
        <hr/>
        <br/>
        <h2 style="text-align: center;">Détails responsable personnel : {{ $nom }}  {{ $prenom }}</h2>
        <br/>
        <div>
            <?php $url = 'storage/trashImages/responsable_personnel/'.$photo;
                $path= public_path($url);
                if(! file_exists($path) || $photo === null){
                    $path= public_path('storage/trashImages/responsable_personnel/default.jpeg');
                }
            ?>
