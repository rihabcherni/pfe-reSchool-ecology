...
<html lang="en">
    ...
    <body class="page"  >
        ...
        <hr/>
        <br/>
        <h2 style="text-align: center;">Détails responsable commerciale: {{ $nom }}  {{ $prenom }}</h2>
        <br/>
        <div>
            <?php $url = 'storage/trashImages/responsable_commercial/'.$photo;
                $path= public_path($url);
                if(! file_exists($path) || $photo === null){
                    $path= public_path('storage/trashImages/responsable_commercial/default.jpeg');
                }
            ?>
            <img class="img-container" src="{{$path }}" alt="responsable commercial"/>
            <table class="table">
                <tr>
                    <th>Identifiant:</th>
                    <td>{{$id}}</td>
