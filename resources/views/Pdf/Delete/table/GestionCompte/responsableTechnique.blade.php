...
<html lang="en">
    ...
    <body class="page">
        ...
        <hr/>
        <br/>
        <h2 style="text-align: center;">Liste des responsables technique de Reschool Ecology supprimés: </h2>
        <br/>
        <table>
            <tr>
                <th>Id</th>
            ...
            <tr>
                <td> {{ $l['id'] }}</td>
                <td>
                    <?php $url = 'storage/trashImages/responsable_technique/'.$l['photo'];
                    $path= public_path($url);
                    if(! file_exists($path) || $l['photo'] === null){
                        $path= public_path('storage/trashImages/responsable_technique/default.jpeg');
                    }
                    ?>
                    <img class="img-container" src="{{$path }}" alt="responsable technique"/>
                </td>
                <td> {{ $l['nom'] }}</td>
                <td> {{ $l['prenom'] }}</td>
                <td> {{ $l['CIN'] }}</td>
